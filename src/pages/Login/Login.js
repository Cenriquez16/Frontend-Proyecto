...
//          window.location.assign('http://localhost:3000/DatosCartera')
//      }else
//      {
//      window.alert("Usuario o contraseña incorrectos");
//      }
//  }
    ...
    inicio = async () => {
    //    valit();
       await axios.get(`${process.env.REACT_APP_API}/api/login`, { params: { username: this.state.Form.username, password: md5(this.state.Form.password) } })
           .then(res => {
     ...
     window.alert("Usuario o contraseña incorrectos");
       window.location.assign(`${process.env.REACT_APP_API}/`);
   }
})
           .catch(err => {
               console.log(err);
